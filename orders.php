...
// Fetch user's orders
$orders_query = "SELECT id, total_amount, status, created_at FROM orders WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?";
...
<head>
    ...
    <title>My Orders - BookCommerce</title>
                    ...
                    <?php foreach ($orders as $order): ?>
                        <div class="flex justify-between items-center p-4 bg-gray-50 rounded-md">
                            <div>
                                ...
                                <p class="text-xs text-gray-600"><?php echo date('F d, Y H:i', strtotime($order['created_at'])); ?></p>
                            </div>
